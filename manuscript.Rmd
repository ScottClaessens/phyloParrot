---
title             : "Evidence of self-care tooling and phylogenetic modelling reveal parrot tool use is not rare"
...
capabilities [@Audet2024;@Lefebvre2002;@Lefebvre1997;@Lefebvre2004]. These 
...
in changing environments [@Sol2005;@Sol2009] (but see Kaplan[@Kaplan2020]). 
...
technical innovations to expand their dietary niche 
[@Ducatez2015;@Overington2011;@Sol2005] and others arguing that feeding 
specialists require technical innovations for extractive foraging of specific 
foods [@HenkeVonDerMalsburg2020;@MettkeHofmann2014].
...
platforms [@Nelson2013]. With millions of videos posted every day, digital 
...
of rare animal behaviours, including interspecies play in dogs [@Nelson2013], 
novel problem-solving behaviours in horses [@Krueger2019], and death-related 
behaviours in Asian elephants [@Pokharel2022]. By casting the net wider than the 
...
could _still_ remain unobserved. One principled framework for identifying these
...
undetected. Furthermore, based on existing theories of the evolution of tool use
[@Ducatez2015;@HenkeVonDerMalsburg2020;@Lefebvre1997;@Lefebvre2002;
@Lefebvre2004;@MettkeHofmann2014;@Overington2011;@Sol2005;@Sol2009], we propose
...
3%) use tools [@Auersperg2009;@Auersperg2010;@Auersperg2011a;@Auersperg2011b;
@Auersperg2012;@Bastos2021;@BentleyCondit2010;@Borsari2005;@Goodman2018;
@Heinsohn2017;@Janzen1976;@Lambert2015;@Lefebvre2002;@OHara2021]. Parrot tool 
...
@Young2012], enabling us to leverage the power of digital video platforms to 
search for evidence of tool use [@Nelson2013]. Third, detailed data on relative
brain sizes [@Flammer2001;@Iwaniuk2005;@Mazengenya2018;@Olkowicz2016;
@Schuck2008;@Silva2017], feeding strategies [@Wilman2016], and shared ancestry
[@Jetz2012] exist for parrots, allowing us to fit the statistical model implied
...
(_i_) rank further parrot species that are likely unobserved tool users and 
(_ii_) re-examine key hypotheses regarding the evolutionary drivers and origins
of tool use in parrots.

# Results

## Digital video platform reveals self-care tool use in additional parrot species
...
```{r echo=F}
tar_load(crowdData)
trueCrowdData <- dplyr::filter(crowdData, ToolUse == "Y")
bordCrowdData <- dplyr::filter(crowdData, ToolUse == "B")
```
...
wood, cardboard) were also common. In `r sum(readxl::read_xlsx(tar_read(crowdsourcing_file))$Directionality == "Tool to body", na.rm = TRUE)` 
...
by Shumaker et al.[@Shumaker2011], compared to the 11 previously reported.
...
other tool behaviours that fit the criteria for true tool use [@Shumaker2011].
By analysing these observations alongside data on relative brain size, feeding
strategy, and phylogenetic ancestry, we can (_i_) identify which parrot species
without recorded evidence of tool use are most likely to have a capacity for 
tool use that has as yet gone unobserved, and (_ii_) test theories about the 
...
`r printnum(tar_read(auc), digits = 2)` (Figure 
...
ophthalmica_). This species is endemic to Papua New Guinea and is relatively 
...
This species is also found in the _Cacatua_ genus, a 
...
and the gang-gang cockatoo (_Callocephalon fimbriatum_). Figure 2 
...
ancestor of amazon parrots native to the Americas (_Amazona_), the most recent 
common ancestor of the true white cockatoos and corellas found in South East 
Asia and Australasia (_Cacatua_), the most recent common ancestor of the kea and 
the kākā from New Zealand (_Nestor_), and the most recent common ancestor for
...
28 species (7%). These species consisted of cockatoos (_Cacatuidae_), Old World
parrots (_Psittacinae_), and neotropical parrots (_Arinae_). Beyond the video 
...
extension of the animal's body to achieve a goal [@Shumaker2011]. However, a 
...
extractive foraging and courtship [@Goodman2018;@Heinsohn2017;@OHara2021;
@Wood1984], parrots in captivity rarely ever require tools in an environment 
...
interface by using an object to manipulate another target or surface 
[@Fragaszy2018]. In our dataset, most self-scratching tooling instances involved 
...
purposes in the wild [@Auersperg2012;@Bastos2021;@Goodman2018;@OHara2021]. For 
...
suggested by studies on welfare and chronic stress in lone captive parrots 
[@Aydinonat2014;@Engebretson2006;@Mason2010;@Meehan2003]. Beyond social
...
of a missing upper mandible [@Bastos2021]. These behaviours might therefore be 
...
ancestors for the _Amazona_, _Cacatua_, _Nestor_, and _Poicephalus_ genera. The 
...
hypothesis for the observed tool use in _Cacatua_ [@Auersperg2012;@OHara2021] 
and _Nestor_ [@Auersperg2009;@Auersperg2010;@Auersperg2011a;@Auersperg2011b;
...
technical intelligence in parrots, e.g., Cabrera-Álvarez and Clayton
[@Alvarez2020]. In our phylogenetic model, we also found that tool use was 
...
to expand a generalist species' dietary niche [@HenkeVonDerMalsburg2020;
@HenkeVonDerMalsburg2021;@Ducatez2015]. However, the trend contradicts theories
...
especially prone to using tools [@HenkeVonDerMalsburg2020].
...
record, such as species abundance and geographic accessibility [@Ducatez2014].
...
observed behaviours, including tool use in other taxa [@Hunt2013], rhythmic 
entrainment in birds [@Benichov2016;@Patel2009;@tenCate2016;@Wilson2016],
teasing behaviours in primates [@Eckert2020], and tactical deception across all
animals [@BroJorgensen2010;@Byrne1988;@Byrne1991]. We hope that these methods
...
reliability [@Auersperg2020;@DechaumeMoncharmont2020;@Farrar2020;@Nelson2013;@Sandor2020;@vonBayern2020]. 
...
suggesting that their behaviour was not random or accidental [@Sandor2020]. 
...
_Figure 1. Causal model of observed tool use._ Directed acyclic graph 
...
_Figure 2. Results of video survey and phylogenetic survival cure modelling mapped onto a maximum clade credibility phylogeny of the parrot order._
...
to a lack of genomic data: _Psittacara erythrogenys_, _Psittacus timneh_, and 
_Aratinga nenday_). Blue points in the outer ring indicate species observed in
...
observed in the scientific literature or the video survey. Total _n_ = 
`r nrow(tar_read(d))` species.

_Figure 3. Posterior predicted probabilities of tool use for each species from our phylogenetic survival cure model._ 
Points are posterior medians and lines are 50% and 95% credible intervals. 
Total _n_ = `r nrow(tar_read(d))` species.

_Figure 4. Expected number of published papers and videos until tool use discovery, according to the survival component of the phylogenetic survival cure model._ 
Densities are full posterior distributions, points are 
posterior medians, and lines are 50% and 95% credible intervals.

_Figure 5. Posterior predictions for the effects of feeding strategy and encephalisation quotient on the probability of tool use from the phylogenetic survival cure model._ 
